...
void interrupt(int arg)
{
	...
	for (int i = 0; i < MAX_CLIENT_CNT; i++)
	{
		if (client_sock[i] != 0)
		{
			pthread_cancel(tid[i]);  // 스레드 종료 요청
			pthread_join(tid[i], 0); // 스레드 종료대기
			close(client_sock[i]);
		}
	}
	close(server_sock); // 이후 서버 소켓 종료
	exit(1);
}
...
// 접속한 클라이언트가 어느 소켓에 들어가야할지 적잘한 위치 반환
int getClientID()
{
	for (int i = 0; i < MAX_CLIENT_CNT; i++)
	{
...
int main()
{
	...
	// 5개의 클라이언트가 어떤 아이디에 배정되었는지 알기 위함
